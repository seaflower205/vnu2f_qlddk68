...
class PlotDataParser:
    ...
    def parse_coordinate_file(file_path, file_type=None, delimiter=',', has_header=True, sheet_name=None):
        ...
        if file_type == 'excel':
            return PlotDataParser._parse_excel(file_path, sheet_name, has_header)
        elif file_type == 'gpx':
            return PlotDataParser._parse_gpx(file_path)
        else:
            return PlotDataParser._parse_text(file_path, delimiter, has_header)
    ...
    def _parse_excel(file_path, sheet_name=None, has_header=True):
        try:
            ...
            return PlotDataParser._dataframe_to_result(df, has_header)
        except Exception as e:  # noqa: BLE001 — intentional suppress
    ...
    def _parse_gpx(file_path):
        try:
            ...
            return PlotDataParser._dataframe_to_result(df, has_header=True)
        except Exception as e:  # noqa: BLE001 — intentional suppress
